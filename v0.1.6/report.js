...
function render() {
  ...
  for (const run of state.runs) {
    ...
    option.value = run.id;
    option.textContent = `${run.workflowId || "未知工作流"} / ${run.traceId || run.logId || run.executeId || "无运行ID"} / ${new Date(run.capturedAt).toLocaleString()}`;
  ...
  summary.textContent = `${run.records.length} 条接口记录，${run.nodes.length} 条节点详情`;
