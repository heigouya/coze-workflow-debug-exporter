...
const CLEARED_KEY = "coze-debug-cleared-at";
...
async function load() {
  ...
  if (!state.selectedRunId && runs[0]) state.selectedRunId = runs[0].id;
  render();
}
...
function render() {
  ...
  const runs = visibleRuns();
  // 选中的运行被过滤掉时，回退到第一条可见运行
  if (!runs.some((run) => run.id === state.selectedRunId)) {
    state.selectedRunId = runs[0] ? runs[0].id : "";
  }
  ...
  summary.textContent = `本会话 ${runs.length} 次日志；当前 ${run.records.length} 条接口记录，${run.nodes.length} 条节点详情`;
  layoutGallery();
}
...
/* ---------- 只显示本会话新抓的运行（过滤历史列表带出来的旧运行） ---------- */

function clearedAt() {
  const v = Number(localStorage.getItem(CLEARED_KEY) || 0);
  return Number.isFinite(v) ? v : 0;
}

function runTimeMs(run) {
  const t = run.startedAt || run.capturedAt || run.timestamp;
  if (t == null) return 0;
  if (typeof t === "number") return t;
  const numeric = Number(t);
  if (Number.isFinite(numeric) && String(t).trim() !== "") return numeric;
  const parsed = Date.parse(t);
  return Number.isNaN(parsed) ? 0 : parsed;
}

// 清空后只保留「运行开始时间 >= 清空时刻」的运行；未清空过则全部显示
function visibleRuns() {
  const threshold = clearedAt();
  if (!threshold) return state.runs;
  return state.runs.filter((run) => runTimeMs(run) >= threshold);
}

function currentRun() {
  return visibleRuns().find((run) => run.id === state.selectedRunId);
}
