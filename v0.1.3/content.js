(function () {
  ...
  function collectNodeLabels() {
    ...
    const nodes = document.querySelectorAll(
      ".react-flow__node, [data-node-id], [data-nodeid], [data-id][class*='node'], [data-id][class*='Node']"
    );

    nodes.forEach((node) => {
      if (isExecutionResultPanel(node)) return;
  ...
  function extractNodeLabel(node, nodeId) {
    const preferred = node.querySelector(
      '[class*="title"], [class*="Title"], [class*="name"], [class*="Name"], [data-testid*="title"], [data-testid*="name"]'
    );
    const candidates = [
  ...
  function isUsefulLabel(value, nodeId) {
    ...
    if (isExecutionResultLabel(value)) return false;
    return /[\u4e00-\u9fa5A-Za-z]/.test(value);
  }

  function isExecutionResultPanel(node) {
    const text = compactText(node.textContent);
    return /运行成功|运行失败|运行中|执行成功|执行失败/.test(text) &&
      !/输入.*输出/.test(text) &&
      !node.matches(".react-flow__node");
  }
