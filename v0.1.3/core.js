(function (root, factory) {
  ...
  function groupRecords(records, options) {
    ...
    for (const record of sorted) {
      const key = runKeyForRecord(record);
      if (!runs.has(key)) {
        runs.set(key, {
          id: key,
          workflowId: record.workflowId,
  ...
  function runKeyForRecord(record) {
    if (record.executeId) return record.executeId;
    if (record.logId) return record.logId;
  ...
  function mergeNode(target, source) {
    if (!target) return;
    target.nodeName = preferNodeName(target.nodeName, source.nodeName);
  ...
  function applyNodeLabel(record, nodeLabels) {
    const label = lookupNodeLabel(record, nodeLabels);
  ...
  function lookupNodeLabel(record, nodeLabels) {
    ...
    return candidates.find((value) => isUsefulNodeName(value, record));
  }

  function preferNodeName(primary, fallback) {
    if (isUsefulNodeName(primary)) return primary;
    if (isUsefulNodeName(fallback)) return fallback;
  ...
  function sanitizeNodeLabels(labels) {
    ...
    return Object.fromEntries(
      Object.entries(labels)
        .map(([nodeId, label]) => [nodeId, String(label || "").trim()])
