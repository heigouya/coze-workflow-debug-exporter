...
function render() {
  ...
  nodes.forEach((node, index) => content.appendChild(renderNodeCard(node, index)));

  // 卡片插入后检测预览是否溢出，溢出的显示渐隐+展开提示
...
function renderNodeCard(node, index) {
  ...
  card.querySelector(".card-preview .json").innerHTML =
    CozeDebuggerCore.renderJsonHtml(preview, false);
...
function buildSection(key, label, value) {
  ...
  if (value === undefined || value === null) {
    ...
    pre.innerHTML = CozeDebuggerCore.renderJsonHtml(value, false);
  }
